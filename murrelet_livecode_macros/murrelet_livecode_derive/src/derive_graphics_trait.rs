...
#[darling(attributes(graphics), supports(any))]
...
pub fn impl_graphics_trait(ast: DeriveInput) -> TokenStream2 {
    ...
    let parsed = match &ast_receiver.data {
        ...
        ast::Data::Struct(fields) => {
            let ctrl = syn::Ident::new(&ast_receiver.ctrl.expect("missing ctrl with graphics type"), ast_receiver.ident.span());
...
fn parse_graphics(name: &syn::Ident, ctrlcls: &syn::Ident, fields: &[LivecodeFieldReceiver]) -> TokenStream2 {
    ...
    for f in fields {
        if let (Some(kind), Some(ident)) = (&f.kind, &f.ident) {
            ...
            match kind {
                ...
                GraphicKind::DrawSrc => {
                    draw_src.push(quote!{self.#ident.gpu_pipelines(render_in)});
                    if let Some(ctrl_) = &f.ctrl {
                        ctrl.push(quote!{self.#ident.control_graphics(&#ctrl_)});
                    }
                },
